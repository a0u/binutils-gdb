/* Common target-dependent code for RISC-V.

   Copyright (C) 2014-2015 Free Software Foundation, Inc.
   Contributed by Todd Snyder <todd@bluespec.com>.
...
static const char *riscv_gdb_reg_names[RISCV_LAST_REGNUM+1] = {
  ...
  "fflags", "frm", "fcsr", "stats", "sup0", "sup1", "epc", "badvaddr",
  "ptbr", "asid", "count", "compare", "evec", "cause", "status",
  "hartid", "impl", "fatc", "send_ipi", "clear_ipi", "reset", "tohost",
		    ...
		    const gdb_byte  *writebuf)
{
  ...
  if (fp) {
    if (riscv_debug)
      fprintf_unfiltered (gdb_stderr, "Return float in fa0\n");
    regnum = RISCV_FA0_REGNUM;
  } else {
    if (riscv_debug)
      fprintf_unfiltered (gdb_stderr, "Return scalar in a0\n");
    regnum = RISCV_A0_REGNUM;
  }
			    ...
			    int                all)
{
  if (regnum != -1) { // print one specified register
    ...
    regnum = 0;
    while (regnum < RISCV_FIRST_FP_REGNUM /*RISCV_LAST_REGNUM*/) {
      if ('\0' == *(riscv_register_name (gdbarch, regnum))) {
	error(_("Not a valid register for the current processor type"));
      }
		     ...
		     struct riscv_frame_cache *this_cache)
{
 ...
 restart:
  ...
  for(cur_pc = start_pc; cur_pc < limit_pc; cur_pc += RISCV_INSTLEN) {
    ...
    else if (opcode == 0x13 && reg == RISCV_FP_REGNUM && rs1 == RISCV_SP_REGNUM) {  // addi s0, sp, size
    ...
    else if ((opcode == 0x33 && reg == RISCV_FP_REGNUM && rs1 == RISCV_SP_REGNUM && rs2 == RISCV_ZERO_REGNUM) ||
	     (opcode == 0x3B && reg == RISCV_FP_REGNUM && rs1 == RISCV_SP_REGNUM && rs2 == RISCV_ZERO_REGNUM)) { // add s0, sp, 0   addw s0, sp, 0
    ...
    else if (opcode == 0x23 && funct3 == 0x2 && rs1 == RISCV_FP_REGNUM) { // sw reg, offset(s0)
